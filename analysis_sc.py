# OVERWINTERING FIRE DETECTION ANALYSIS: Spatial Continuity Criteria
...
#
# Spatial Continuity: Spring hotspot selection: fall proximity first, then perimeter
#   distance checked as a secondary gate.
#   best_spring is selected as the hotspot closest to the last fall hotspot
#   (Scholten spatial continuity). It is then rejected if it is not within
#   MAX_PERIM_DIST_M of a current-year perimeter.
#
# Spatial Continuity criteria (all must be satisfied):
...
#   3. That spring hotspot is within MAX_FALL_SPRING_DIST_M of the last fall hotspot
#   4. Same spring hotspot is within MAX_PERIM_DIST_M of a current-year perimeter
#   5. Spring-year perimeter fire_cause != 'Person'
#   6. No CG lightning strike within 7 days before spring hotspot detection
#
...
MAX_FALL_SPRING_DIST_M = 1000     # max straight-line distance from last fall to spring hotspot
...
def detect_overwintering(fires, buffered_dfs, fall_hotspots, spring_hotspots,
                         ...
                         max_fall_spring_dist_m=MAX_FALL_SPRING_DIST_M):
    """
    Detect overwintering fires — OPTION B.

    Spring hotspot selection: fall proximity first, perimeter distance as gate.
      1. Find all spring hotspots in buffer within SDD window
      2. Select best_spring as the hotspot closest to the last fall hotspot
         (Scholten spatial continuity — physically anchored to dormant fire)
      3. GATE: reject best_spring if fall-to-spring distance exceeds
         max_fall_spring_dist_m
      4. GATE: reject best_spring if it is not within max_perim_dist_m of
         any current-year fire perimeter (perimeter linkage criterion)
    ...
    max_fall_spring_dist_m : int
        Max straight-line distance from last fall hotspot to spring hotspot
    ...
    for _, fire in perims.iterrows():
        ...
        fall_to_spring_dist = None

        if has_spring and lf_x is not None:
            spring_in_buffer = spring_in_buffer.copy()

            # --- STEP 1: select hotspot closest to last fall hotspot ---
            # vectorised fall-to-spring distances in BC Albers metres
            sp_lons = spring_in_buffer['longitude'].values
            sp_lats = spring_in_buffer['latitude'].values
            sp_xs, sp_ys = TRANSFORMER.transform(sp_lons, sp_lats)
            spring_in_buffer['fall_to_spring_dist'] = [
                euclidean([lf_x, lf_y], [sx, sy])
                for sx, sy in zip(sp_xs, sp_ys)
            ]

            # select closest spring hotspot to last fall hotspot
            best_idx    = spring_in_buffer['fall_to_spring_dist'].idxmin()
            best_spring = spring_in_buffer.loc[best_idx]
            fall_to_spring_dist = best_spring['fall_to_spring_dist']

            # --- STEP 2: gate — fall-to-spring travel distance ---
            if fall_to_spring_dist <= max_fall_spring_dist_m:

                # --- STEP 3: gate — perimeter proximity ---
                dist_to_perim = current_perims['geometry'].distance(
                    best_spring['geometry']
                ).min() if not current_perims.empty else np.inf

                nearest_perim_id = current_perims.loc[
                    current_perims['geometry'].distance(best_spring['geometry']).idxmin(),
                    'fire_id'
                ] if not current_perims.empty else None

                if dist_to_perim <= max_perim_dist_m:
                    has_spring_near_perim      = True
                    spring_near_perim          = spring_in_buffer[
                        spring_in_buffer['fall_to_spring_dist'] <= max_fall_spring_dist_m
                    ]
                    first_spring_date          = best_spring['datetime']
                    first_spring_lat           = best_spring['latitude']
                    first_spring_lon           = best_spring['longitude']
                    first_spring_dist_to_perim = dist_to_perim
                    linked_perim_id            = nearest_perim_id
                    dormancy_days              = (
        ...
        results.append({
            ...
            'n_spring_near_perim':        len(spring_near_perim),
            ...
            'fall_to_spring_dist_m':      fall_to_spring_dist,
